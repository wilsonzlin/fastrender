...
fn bench_display_list_creation(c: &mut Criterion) {
    ...
    for count in [10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::new("complex_list", count), count, |b, &count| {
            b.iter(|| create_complex_display_list(black_box(count)))
        });
    }
...
fn bench_display_item_creation(c: &mut Criterion) {
    ...
    group.bench_function("fill_rect", |b| {
        b.iter(|| {
            DisplayItem::FillRect(FillRectItem {
                ...
                color: Rgba::new(black_box(100), black_box(150), black_box(200), black_box(1.0)),
            })
        })
    });
    ...
    group.bench_function("rgba_creation", |b| {
        b.iter(|| Rgba::new(black_box(100), black_box(150), black_box(200), black_box(1.0)))
    });
...
fn bench_fragment_tree_operations(c: &mut Criterion) {
    ...
    group.bench_function("new_block_fragment", |b| {
        b.iter(|| {
            FragmentNode::new_block(
                Rect::from_xywh(black_box(10.0), black_box(20.0), black_box(100.0), black_box(50.0)),
                vec![],
            )
        })
    });
    ...
    group.bench_function("new_text_fragment", |b| {
        b.iter(|| {
            FragmentNode::new_text(
                Rect::from_xywh(black_box(10.0), black_box(20.0), black_box(200.0), black_box(16.0)),
