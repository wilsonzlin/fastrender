...
impl BoxGenerator {
  ...
  fn should_skip_element(&self, node: &DOMNode) -> bool {
    // Always skip non-rendered metadata elements per HTML UA defaults.
    // These never generate boxes and their contents should not be rendered.
...
mod tests {
  ...
  fn ordered_list_start_attribute_overrides_ua_counter_reset() {
    ...
    let markers: Vec<String> = BoxGenerator::find_boxes_by_predicate(&tree.root, |b| {
      matches!(b.box_type, BoxType::Marker(_))
    })
    .iter()
  ...
  fn reversed_ordered_list_counts_down_with_ua_defaults() {
    let dom =
      crate::dom::parse_html(r"<ol reversed><li>one</li><li>two</li><li>three</li></ol>").unwrap();
    let styled = crate::style::cascade::apply_styles(&dom, &crate::css::types::StyleSheet::new());
    let tree = generate_box_tree(&styled);
  ...
  fn ordered_list_value_attribute_respected_with_ua_defaults() {
    let dom =
      crate::dom::parse_html(r#"<ol start="5"><li value="10">one</li><li>two</li></ol>"#).unwrap();
    let styled = crate::style::cascade::apply_styles(&dom, &crate::css::types::StyleSheet::new());
    let tree = generate_box_tree(&styled);
  ...
  fn reversed_list_value_attribute_counts_down_with_ua_defaults() {
    let dom = crate::dom::parse_html(
      r#"<ol reversed><li value="5">one</li><li>two</li><li>three</li></ol>"#,
    )
    .unwrap();
    let styled = crate::style::cascade::apply_styles(&dom, &crate::css::types::StyleSheet::new());
    let tree = generate_box_tree(&styled);
  ...
  #[test]
  fn legacy_lists_reset_counters() {
    let generator = BoxGenerator::new();
    ...
    // Inspect menu markers
    let menu_box = tree.root.children[1].clone();
    ...
    assert_eq!(first_marker_text, "1 ");
    assert_eq!(second_marker_text, "2 ");
  }
