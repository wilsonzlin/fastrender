...
mod tests {
    ...
    fn oversized_image_allows_negative_alignment_space() {
        ...
        let (offset_x, offset_y, dest_w, dest_h) =
            compute_object_fit(ObjectFit::None, position, 100.0, 100.0, 200.0, 50.0).expect("fit computed");
    ...
    fn percentage_position_uses_alignment_space() {
        ...
        let (offset_x, offset_y, _, _) =
            compute_object_fit(ObjectFit::Contain, position, 120.0, 60.0, 60.0, 60.0).expect("fit computed");
