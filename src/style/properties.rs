...
fn apply_property_from_source(styles: &mut ComputedStyle, source: &ComputedStyle, property: &str, order: i32) -> bool {
...
fn apply_global_keyword(
    ...
    let Some(source) = global_keyword_source(keyword, property, parent, defaults, revert_base, revert_layer_base)
    else {
        return false;
    };
            ...
            if let Some(values) = parse_layer_list(&resolved_value, parse_background_size_component) {
                ...
                for idx in 0..layer_count {
                    ...
                    let source = styles
                        ...
                        .cloned()
                        .unwrap_or(default);
                    let inline_val = values
                        .get(idx)
                        .cloned()
                        .unwrap_or_else(|| values.last().copied().unwrap_or(BackgroundSizeComponent::Auto));
                    let (mut w, mut h) = match source {
                        BackgroundSize::Explicit(w, h) => (w, h),
                        BackgroundSize::Keyword(_) => (BackgroundSizeComponent::Auto, BackgroundSizeComponent::Auto),
                    };
                    if horizontal_inline {
                        w = inline_val;
                    } else {
                    ...
                    sizes.push(BackgroundSize::Explicit(w, h));
                }
                styles.background_sizes = sizes;
                styles.rebuild_background_layers();
            }
