    ...
    pub fn add_float(&mut self, float_info: FloatInfo) {
        match float_info.side {
            FloatSide::Left => {
                ...
                self.left_floats.sort_by(|a, b| a.top().partial_cmp(&b.top()).unwrap());
            }
            FloatSide::Right => {
                self.right_floats.push(float_info);
                self.right_floats.sort_by(|a, b| a.top().partial_cmp(&b.top()).unwrap());
            }
        }
    }
...
mod tests {
    ...
    fn test_float_side_from_float() {
        assert_eq!(Option::<FloatSide>::from(Float::Left), Some(FloatSide::Left));
        assert_eq!(Option::<FloatSide>::from(Float::Right), Some(FloatSide::Right));
