...
impl LineMetrics {
  ...
  pub fn from_strut(strut_metrics: &ScaledMetrics) -> Self {
    let half_leading =
      (strut_metrics.line_height - (strut_metrics.ascent + strut_metrics.descent)) / 2.0;
    Self {
...
mod tests {
  ...
  fn test_baseline_aligner_different_sizes() {
    ...
    assert_eq!(
      boxes[0].y_offset + boxes[0].metrics.ascent,
      line_metrics.baseline
    );
    assert_eq!(
      boxes[1].y_offset + boxes[1].metrics.ascent,
      line_metrics.baseline
    );
  }
