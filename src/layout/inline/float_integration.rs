...
impl<'a> InlineFloatIntegration<'a> {
    ...
    pub fn find_line_space(&self, start_y: f32, options: LineSpaceOptions) -> LineSpace {
        ...
        let y = if options.line_height > 0.0 {
            self.float_ctx.find_fit(options.min_width, options.line_height, start_y)
        } else {
        ...
        let (left_edge, width) = if options.line_height > 0.0 {
            self.float_ctx.available_width_in_range(y, y + options.line_height)
        } else {
...
impl<'a> InlineFloatIntegrationMut<'a> {
    ...
    pub fn place_inline_float(
        ...
        let (x, y) = self.float_ctx.compute_float_position(side, width, height, min_y);
...
mod tests {
    ...
    fn test_line_space_options_builder() {
        let opts = LineSpaceOptions::default().min_width(100.0).line_height(20.0);
