...
static HISTOGRAM: OnceLock<std::sync::Mutex<std::collections::HashMap<(usize, (Option<u32>, Option<u32>)), u64>>> =
    OnceLock::new();
...
fn enabled() -> bool {
    *ENABLED.get_or_init(|| std::env::var("FASTR_FLEX_PROFILE").map(|v| v != "0").unwrap_or(false))
}
...
fn maybe_log_progress(current: u64) {
    let Some(interval) = progress_interval() else { return };
...
pub fn log_flex_profile(total: Duration) {
    ...
    if enabled {
        eprintln!(
            ...
            hit_buckets.get(0).copied().unwrap_or(0),
            ...
            bucket_rates.get(0).copied().unwrap_or(0.0),
                ...
                if !top_n.is_empty() {
                    let formatted: Vec<String> = top_n
                        ...
                        .map(|((bucket, key), count)| {
                            format!("bucket={} key=({:?},{:?}) count={}", bucket, key.0, key.1, count)
                        })
                        .collect();
...
fn log_node_profile() {
    ...
    let summary: Vec<String> = entries
        ...
        .collect();
    let mut lookup_entries: Vec<_> = guard.iter().filter(|(_, stats)| stats.lookups > 0).collect();
        ...
        if let Some(map) = HISTOGRAM.get() {
            if let Ok(guard) = map.lock() {
                if !guard.is_empty() {
                    ...
                    eprintln!("flex profile histogram top10: {}", top.collect::<Vec<_>>().join(" | "));
                }
            }
        }
    }
