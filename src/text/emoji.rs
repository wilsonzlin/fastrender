...
pub fn is_emoji_modifier_base(c: char) -> bool {
    ...
    // More people emoji
    if (0x1F47C..=0x1F47C).contains(&cp) || (0x1F481..=0x1F487).contains(&cp) || (0x1F4AA..=0x1F4AA).contains(&cp) {
        return true;
    }
...
mod tests {
    ...
    fn test_find_emoji_sequences_multiple_flags() {
        let seqs = find_emoji_sequences("🇺🇸🇬🇧🇯🇵");
        assert_eq!(seqs.len(), 3);
        assert!(seqs.iter().all(|s| s.sequence_type == EmojiSequenceType::FlagSequence));
    }
