...
impl TextShaper {
  ...
  pub fn shape_text(
    ...
    let rb_face =
      Face::from_slice(&font.data, font.index).ok_or_else(|| TextError::ShapingFailed {
  ...
  fn build_clusters(&self, text: &str, glyphs: &[GlyphPosition]) -> Vec<GlyphCluster> {
    ...
    let mut current_cluster_start_byte =
      self.clamp_to_prev_boundary(text, glyphs[0].cluster as usize);
