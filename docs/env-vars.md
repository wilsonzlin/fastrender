...
- `FASTR_PAINT_PARALLEL=0|1` – enable/disable tiled parallel rasterization when painting display lists (default enabled with adaptive thresholds).
- `FASTR_LAYOUT_PARALLEL=off|on|auto` – override layout fan-out mode regardless of RenderOptions/FastRenderConfig (default off).
...
- `FASTR_LAYOUT_PARALLEL_MIN_NODES=<N>` – minimum box nodes before auto layout fan-out will engage (default 512).
