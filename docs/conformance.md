...
| Text | Shaping pipeline | ⚠️ | [src/text/pipeline.rs](../src/text/pipeline.rs) | [tests/text/pipeline_test.rs](../tests/text/pipeline_test.rs) | UAX #9 bidi, script itemization, RustyBuzz shaping with OpenType features/variations. Color glyph path prefers CBDT/CBLC+sbix PNG bitmaps → SVG-in-OT → COLR/CPAL v0 solids via [`text/color_fonts.rs`](../src/text/color_fonts.rs); COLR v1 paint graph unsupported (outline fallback). `palette_index` on shaped runs is always 0 (no `font-palette`/`@font-palette-values`, CPAL v1 palette flags ignored). See [notes/color-fonts.md](notes/color-fonts.md). |
