...
This task implements a greedy line breaking algorithm for the FastRender V2 text rendering system, integrated into the existing `line_break` module. The implementation adds `Line` and `LineSegment` types and a greedy algorithm that fits as much text as possible on each line, breaking at valid opportunities when the line is full.

The algorithm uses the existing `GlyphPosition` type from `shaper.rs` and the existing `BreakOpportunity`/`BreakType` types from the UAX #14 implementation in `line_break.rs`. This integration ensures consistency across the text shaping pipeline.

The algorithm follows CSS Text Module Level 3 specification and integrates with the Unicode Line Breaking Algorithm (UAX #14). It handles:
- Normal breaks (whitespace, word boundaries) - `BreakType::Allowed`
- Hard/mandatory breaks (newlines, `<br>`) - `BreakType::Mandatory`
- Overflow when no break opportunities exist

## API Contracts

### Core Types (Using Existing Infrastructure)

**GlyphPosition** (from `shaper.rs`):
```rust
pub struct GlyphPosition {
    pub glyph_id: u32,      // Font-specific glyph ID
    pub cluster: u32,       // Text cluster index (byte offset in original text)
    pub advance: f32,       // Horizontal advance width
    pub advance_y: f32,     // Vertical advance (for vertical text)
    pub offset_x: f32,      // X offset from pen position
    pub offset_y: f32,      // Y offset from pen position
}
```

**BreakOpportunity** and **BreakType** (from `line_break.rs` UAX #14):
```rust
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BreakType {
    Mandatory,  // Hard break (newline, must break here)
    Allowed,    // Soft break (space, may break here)
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct BreakOpportunity {
    pub byte_offset: usize,  // Byte position in UTF-8 text
...
**New Types Added to `line_break.rs`**:
...
|------|-------|-------------|
| `src/text/line_break.rs` | ~390 | Added greedy line breaking to existing UAX #14 module |
| `docs/tasks/notes/W4.T09-notes.md` | This file | Task documentation |

### Integration Changes

- **Removed**: Duplicate `line_breaker.rs` module
- **Removed**: Separate test files `tests/text/test_line_breaker.rs`
- **Modified**: `src/text/mod.rs` - Updated re-exports
- **Modified**: `src/text/line_break.rs` - Added greedy algorithm to existing module

The implementation properly integrates with the existing Wave 4 text shaping pipeline.
