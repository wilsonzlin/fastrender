...
| `background-size` | P1 | ⚠️ Partial | CSS3 Backgrounds | Painting | Parses including logical inline/block; image painting not implemented; regressions in `tests/style/background_position_logical_test.rs` (inline/block longhands and background-size-x/y) |
