...
pipeline uses `ComputedStyle.custom_properties: HashMap<String, String>` plus
...
uses: store custom properties in a `HashMap<String, String>` and resolve
`var()` references with `style::var_resolution::{resolve_var, resolve_var_for_property}`
