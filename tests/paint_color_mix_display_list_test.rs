...
fn color_mix_srgb_matches_resolved_color() {
    let expected = Color::parse("color-mix(in srgb, red 25%, blue 75%)")
        .unwrap()
        .to_rgba(Rgba::BLACK);
    assert_eq!(render_background("color-mix(in srgb, red 25%, blue 75%)"), (expected.r, expected.g, expected.b, expected.alpha_u8()));
}
...
fn color_mix_srgb_linear_matches_resolved_color() {
    let expected = Color::parse("color-mix(in srgb-linear, red 50%, blue 50%)")
        .unwrap()
        .to_rgba(Rgba::BLACK);
    assert_eq!(render_background("color-mix(in srgb-linear, red 50%, blue 50%)"), (expected.r, expected.g, expected.b, expected.alpha_u8()));
}
...
fn color_mix_uses_current_color() {
    ...
    let expected = Color::parse("color-mix(in srgb, red 50%, blue 50%)")
        .unwrap()
        .to_rgba(Rgba::BLACK);
    assert_eq!(pixel(&pixmap, 0, 0), (expected.r, expected.g, expected.b, expected.alpha_u8()));
}
