...
fn save_or_compare(name: &str, raster: &fastrender::text::color_fonts::ColorGlyphRaster) {
  ...
  if std::env::var("UPDATE_GOLDEN").is_ok() {
    raster.image.save_png(&path).expect("failed to write golden");
  }
...
fn malformed_colrv1_table_falls_back() {
  ...
  let gid = face.glyph_index('A').unwrap();
  let rendered = ColorFontRenderer::new()
    .render(&corrupted, gid.0 as u32, 64.0, 0, Rgba::BLACK, 0.0);
...
fn find_table(data: &[u8], tag: &[u8; 4]) -> Option<(usize, usize)> {
  ...
  for _ in 0..num_tables {
    ...
    let length = u32::from_be_bytes([data[offset + 12], data[offset + 13], data[offset + 14], data[offset + 15]]) as usize;
    if table_tag == tag {
