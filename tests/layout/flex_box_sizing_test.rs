...
fn flex_item_auto_min_size_clamps_to_min_content() {
    ...
    let container = BoxNode::new_block(Arc::new(container_style), FormattingContextType::Flex, vec![child_box]);
...
fn flex_item_auto_min_size_column_uses_block_min_content() {
    ...
    let text = BoxNode::new_text(Arc::new(ComputedStyle::default()), "TallContentLine".to_string());
    let child_box = BoxNode::new_block(Arc::new(child_style), FormattingContextType::Block, vec![text]);
    let child_for_intrinsic = child_box.clone();
    ...
    let fragment = fc
        .layout(
            &container,
            &LayoutConstraints::new(AvailableSpace::Definite(200.0), AvailableSpace::Definite(10.0)),
        )
        .expect("layout should succeed");
