...
fn float_auto_width_clamps_to_max_width() {
    ...
    let text_child = BoxNode::new_text(Arc::new(ComputedStyle::default()), "Hello world".to_string());
    let float_box = BoxNode::new_block(Arc::new(float_style), FormattingContextType::Block, vec![text_child]);
