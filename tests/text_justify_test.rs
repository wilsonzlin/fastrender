...
use fastrender::text::justify::{
    ...
    justify_line_with_options, justify_lines, mark_word_boundaries, mark_word_boundaries_by_glyph_id, GlyphPosition,
...
fn glyph_with_cluster(id: u16, x: f32, advance: f32, is_word_boundary: bool, cluster: usize) -> GlyphPosition {
...
fn test_detect_justification_mode_empty() {
    assert_eq!(detect_justification_mode(""), JustificationMode::WordSpacing);
}

#[test]
fn test_detect_justification_mode_whitespace_only() {
    assert_eq!(detect_justification_mode("   \t\n  "), JustificationMode::WordSpacing);
}
...
fn test_justification_result_contains_info() {
    ...
    let result = justify_line_with_options(&mut glyphs, 150.0, 120.0, &JustificationOptions::default());
...
fn test_justify_many_word_boundaries() {
    ...
    assert_approx_eq(glyphs[last_idx].x, 90.0 + 25.0, "Last glyph with many boundaries");
}
