...
mod wpt_runner_tests {
    use super::wpt::{
        HarnessConfig, RunnerStats, SuiteResult, TestMetadata, TestResult, TestStatus, TestType, WptRunner,
        WptRunnerBuilder,
    };
    ...
    fn test_wpt_runner_with_config() {
        let renderer = create_test_renderer();
        let config = HarnessConfig::default().with_tolerance(10).with_max_diff(0.5);
    ...
    fn test_wpt_runner_builder() {
        ...
        assert_eq!(runner.config().expected_dir, PathBuf::from("custom/expected"));
        assert_eq!(runner.config().output_dir, PathBuf::from("target/custom-output"));
    ...
    fn test_type_from_path_reftest() {
        ...
        assert_eq!(TestType::from_path(Path::new("test-ref.htm")), TestType::Reftest);
    }
    ...
    fn test_type_from_path_crashtest() {
        ...
        assert_eq!(TestType::from_path(Path::new("crash-001.html")), TestType::Crashtest);
    }
    ...
    fn test_result_with_images() {
        let metadata = TestMetadata::from_path(PathBuf::from("test.html"));
        let result = TestResult::pass(metadata, Duration::from_millis(100)).with_images(vec![1, 2, 3], vec![4, 5, 6]);
