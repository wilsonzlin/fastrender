...
fn test_style_with_display_block() {
    let style = PseudoElementStyle::with_string("Block", default_style()).with_display(Display::Block);
...
fn test_config_get_before() {
    ...
    assert_eq!(retrieved.unwrap().content.to_text(), Some("Test".to_string()));
}
...
fn test_generate_box_inline_display() {
    let generator = PseudoElementGenerator::new();
    let style = PseudoElementStyle::with_string("Inline", default_style()).with_display(Display::Inline);
...
fn test_generate_box_grid_display() {
    ...
    assert!(box_node.is_block_level());
    assert_eq!(box_node.formatting_context(), Some(FormattingContextType::Grid));
}

#[test]
fn test_generate_box_inline_flex_display() {
    let generator = PseudoElementGenerator::new();
    let style = PseudoElementStyle::with_string("InlineFlex", default_style()).with_display(Display::InlineFlex);
...
fn test_generate_box_inline_grid_display() {
    let generator = PseudoElementGenerator::new();
    let style = PseudoElementStyle::with_string("InlineGrid", default_style()).with_display(Display::InlineGrid);
...
fn test_count_pseudo_boxes_nested() {
    ...
    let inner_before = generator.generate_box(PseudoElementType::Before, &style).unwrap();
    let inner_block = BoxNode::new_block(default_style(), FormattingContextType::Block, vec![inner_before]);

    let outer_after = generator.generate_box(PseudoElementType::After, &style).unwrap();
    let root = BoxNode::new_block(
        default_style(),
        FormattingContextType::Block,
...
fn test_find_pseudo_boxes_with_pseudos() {
    ...
    let root = BoxNode::new_block(default_style(), FormattingContextType::Block, vec![before, after]);
...
fn test_multiple_pseudo_in_siblings() {
    ...
    let before1 = generator.generate_box(PseudoElementType::Before, &style).unwrap();
    let after1 = generator.generate_box(PseudoElementType::After, &style).unwrap();
    let sibling1 = BoxNode::new_block(default_style(), FormattingContextType::Block, vec![before1, after1]);

    let before2 = generator.generate_box(PseudoElementType::Before, &style).unwrap();
    let after2 = generator.generate_box(PseudoElementType::After, &style).unwrap();
    let sibling2 = BoxNode::new_block(default_style(), FormattingContextType::Block, vec![before2, after2]);

    let root = BoxNode::new_block(default_style(), FormattingContextType::Block, vec![sibling1, sibling2]);
...
fn test_css_spec_after_is_last_child() {
    ...
    assert_eq!(get_pseudo_type(&result[last_idx]), Some(PseudoElementType::After));
}
