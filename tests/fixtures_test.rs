...
fn test_fixtures_directory_exists() {
    ...
    assert!(golden_dir().exists(), "Golden images directory should exist");
}
...
fn test_all_fixture_files_exist() {
    ...
    for name in &expected_fixtures {
        let path = html_dir().join(format!("{}.html", name));
        assert!(path.exists(), "Fixture file should exist: {}", path.display());
    }
}
...
pub fn fixture_descriptions() -> Vec<(&'static str, &'static str, &'static str)> {
    vec![
        ...
        ("block_simple", "Block Layout", "Simple stacked block elements"),
        ("block_nested", "Block Layout", "Deeply nested block containers"),
        (
            "block_margin_collapse",
            "Block Layout",
        ...
        ("flex_grow_shrink", "Flexbox", "flex-grow and flex-shrink distribution"),
        (
            "grid_template",
            "Grid",
            "grid-template-columns/rows with various track sizes",
        ),
        ("grid_auto_flow", "Grid", "grid-auto-flow and auto placement"),
        ("grid_gaps", "Grid", "row-gap and column-gap variations"),
        ("table_fixed", "Tables", "table-layout: fixed with explicit widths"),
        ("table_auto", "Tables", "table-layout: auto content-based sizing"),
        ("table_span", "Tables", "colspan and rowspan cell spanning"),
        ("float_basic", "Floats", "Basic left and right floats"),
        ("float_text_wrap", "Floats", "Text wrapping around floated elements"),
        ...
        ("text_bidi", "Text", "Bidirectional text with mixed directions"),
        ("text_line_break", "Text", "Line breaking and white-space handling"),
    ]
}
