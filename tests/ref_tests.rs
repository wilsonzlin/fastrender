...
use r#ref::{
    compare::{compare_images, create_solid_pixmap, CompareConfig, DiffStatistics},
    harness::{RefTestConfig, RefTestHarness, RefTestResult, RefTestResults},
};
...
fn test_ref_harness_render_and_compare_identical() {
    ...
    assert!(!result.passed);
    assert!(result.error.is_some());
    assert!(result.error.as_ref().unwrap().contains("Failed to load reference"));
}
...
fn test_ref_harness_create_reference() {
    ...
    assert!(create_result.is_ok(), "Failed to create reference: {:?}", create_result);
...
fn test_ref_harness_detect_difference() {
    ...
    assert!(result.image_diff.as_ref().unwrap().statistics.different_pixels > 0);
}
