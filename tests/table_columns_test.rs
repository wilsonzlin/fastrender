...
fn test_equal_distribution_two_columns() {
    let columns = vec![ColumnConstraints::new(50.0, 200.0), ColumnConstraints::new(50.0, 200.0)];
...
fn test_equal_distribution_many_columns() {
    let columns: Vec<_> = (0..10).map(|_| ColumnConstraints::new(20.0, 200.0)).collect();
    let distributor = ColumnDistributor::new(DistributionMode::Auto);
...
fn test_fixed_width_exceeds_available() {
    let columns = vec![ColumnConstraints::fixed(200.0), ColumnConstraints::fixed(200.0)];
    let distributor = ColumnDistributor::new(DistributionMode::Auto);
...
fn test_compute_constraints_with_colspan() {
    ...
    let cell_widths = vec![
        vec![(50.0, 100.0, 1_usize), (50.0, 100.0, 1_usize), (50.0, 100.0, 1_usize)],
...
fn test_compute_constraints_uneven_rows() {
    ...
    let cell_widths = vec![vec![(50.0, 100.0, 1), (50.0, 100.0, 1)], vec![(200.0, 400.0, 2)]];
