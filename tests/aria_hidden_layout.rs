...
fn aria_hidden_elements_are_still_laid_out() {
    ...
    assert!(tree.fragment_count() > 1, "aria-hidden content should still generate fragments");
    ...
    let any_hidden = tree
        .iter_fragments()
        .any(|f| f.style.as_ref().map(|s| s.visibility != Visibility::Visible).unwrap_or(false));
    assert!(!any_hidden, "aria-hidden should not force fragments to visibility hidden");
}
