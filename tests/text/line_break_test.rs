...
fn test_no_breaks_in_single_word() {
    ...
    assert!(interior.is_empty(), "Single word should have no interior breaks");
}
...
fn test_newline_creates_mandatory_break() {
    ...
    assert!(mandatory.is_some(), "Should have mandatory break at newline");
...
fn test_trailing_newline() {
    ...
    assert!(!mandatory.is_empty(), "Trailing newline should create mandatory break");
}
...
fn test_word_joiner_prevents_break() {
    ...
    assert!(interior.is_empty(), "Word joiner should prevent interior breaks");
}
...
fn test_chinese_characters_break() {
    ...
    assert!(breaks.len() >= 4, "Chinese text should have multiple breaks");
}
...
fn test_emoji_sequence_no_interior_break() {
    ...
    assert!(interior.is_empty(), "ZWJ emoji sequence should have no interior breaks");
}
...
fn test_flag_emoji_no_interior_break() {
    ...
    assert!(interior.is_empty(), "Flag emoji should have no interior breaks");
}
