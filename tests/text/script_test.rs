...
use fastrender::text::script::{
    is_emoji, is_skin_tone_modifier, is_variation_selector, is_zwj, itemize_scripts, ScriptRun,
};
...
fn test_run_boundaries_valid() {
    ...
    for run in &runs {
        assert_eq!(run.start, expected_start, "Run should start where previous ended");
