...
use fastrender::text::bidi::analyze_bidi;
use fastrender::text::bidi::BidiAnalyzer;
use fastrender::text::bidi::BidiRun;
...
fn override_does_not_leak_to_next_paragraph_with_embed() {
  ...
  assert!(
    first_para.contains("ABC"),
    "override paragraph should keep its content"
  );
  assert!(
    !first_para.contains("CBA"),
    "override should stay local to paragraph"
  );
