...
use fastrender::{find_soft_hyphens, is_soft_hyphen, remove_soft_hyphens, Hyphenator, HyphensMode, SupportedLanguage};
...
fn test_hyphenate_preserves_word() {
    ...
    assert_eq!(reconstructed, word, "Segments should reconstruct original word");
}
...
fn test_hyphenate_short_words() {
    ...
    for word in &short_words {
        ...
        assert!(points.is_empty(), "Short word '{}' should not be hyphenated", word);
    }
}
...
fn test_hyphenate_word_segments() {
    ...
    assert!(segments.len() > 1, "Long word should have multiple segments");
}
...
fn test_hyphenate_empty_string() {
    ...
    assert!(points.is_empty(), "Empty string should have no break points");
}
...
fn test_hyphenate_text_simple() {
    ...
    assert!(!results.is_empty(), "Should find hyphenation points in text");
}
...
fn test_hyphenate_text_no_long_words() {
    ...
    assert!(results.is_empty(), "Short words text should have no hyphenation points");
}
...
fn test_hyphenate_text_positions() {
    ...
    for (word_start, breaks) in &results {
        ...
        for &break_pos in breaks {
            assert!(break_pos <= text.len(), "Break position should be within text");
        }
    }
}
...
fn test_supported_language_parse_english() {
    ...
    assert_eq!(SupportedLanguage::parse("en-GB"), Some(SupportedLanguage::EnglishGB));
}

#[test]
fn test_supported_language_parse_german() {
    assert_eq!(SupportedLanguage::parse("de"), Some(SupportedLanguage::German));
    assert_eq!(SupportedLanguage::parse("de-DE"), Some(SupportedLanguage::German));
}

#[test]
fn test_supported_language_parse_french() {
    assert_eq!(SupportedLanguage::parse("fr"), Some(SupportedLanguage::French));
    assert_eq!(SupportedLanguage::parse("fr-FR"), Some(SupportedLanguage::French));
}
...
fn test_supported_language_all_variants() {
    ...
    let languages = [
        "en", "en-us", "en-gb", "de", "fr", "es", "it", "pt", "pt-br", "pt-pt", "nl", "pl", "ru", "sv", "nb", "da",
        "fi", "hu", "cs", "sk", "hr", "ca", "tr", "el", "uk", "la",
    ];
...
fn test_hyphenate_very_long_word() {
    ...
    assert!(!points.is_empty(), "Very long word should have break points");
}
...
fn test_hyphenator_clone() {
    ...
    assert_eq!(points1, points2, "Cloned hyphenator should behave identically");
}
...
fn test_error_message_contains_language() {
    ...
    assert!(error_string.contains("xyz-lang"), "Error should contain language code");
}
