...
impl TestStatus {
    ...
    pub fn is_failure(&self) -> bool {
        matches!(self, TestStatus::Fail | TestStatus::Error | TestStatus::Timeout)
    }
}
...
impl SuiteResult {
    ...
    pub fn skipped(&self) -> usize {
        self.results.iter().filter(|r| r.status == TestStatus::Skip).count()
    }
...
pub fn compare_images(rendered: &[u8], expected: &[u8], tolerance: u8) -> Result<(u64, u64, f64), String> {
    // Decode PNG images
    let rendered_img = decode_png(rendered).map_err(|e| format!("Failed to decode rendered image: {}", e))?;
    let expected_img = decode_png(expected).map_err(|e| format!("Failed to decode expected image: {}", e))?;
...
pub fn generate_diff_image(rendered: &[u8], expected: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let _rendered_img = decode_png(rendered).map_err(|e| format!("Failed to decode rendered image: {}", e))?;
    let _expected_img = decode_png(expected).map_err(|e| format!("Failed to decode expected image: {}", e))?;
...
mod tests {
    ...
    fn test_test_type_from_path() {
        assert_eq!(TestType::from_path(Path::new("test-ref.html")), TestType::Reftest);
        assert_eq!(TestType::from_path(Path::new("crashtest.html")), TestType::Crashtest);
        assert_eq!(TestType::from_path(Path::new("manual-test.html")), TestType::Manual);
        assert_eq!(TestType::from_path(Path::new("normal-test.html")), TestType::Reftest);
    }
    ...
    fn test_test_metadata_disable() {
        ...
        assert!(metadata.disabled);
        assert_eq!(metadata.disabled_reason, Some("Not implemented".to_string()));
    }
