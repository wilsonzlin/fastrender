...
use super::harness::{
    compare_images, generate_diff_image, HarnessConfig, SuiteResult, TestMetadata, TestResult, TestStatus, TestType,
};
...
impl WptRunner {
    ...
    fn compare_and_result(
        ...
        match compare_images(&rendered, &expected, self.config.pixel_tolerance) {
            ...
            Err(e) => TestResult::error(metadata.clone(), duration, format!("Comparison error: {}", e)),
        }
    }
    ...
    fn save_artifact(&self, result: &TestResult) {
        ...
        if result.status == TestStatus::Fail && self.config.save_diffs {
            if let (Some(ref rendered), Some(ref expected)) = (&result.rendered_image, &result.expected_image) {
...
mod tests {
    ...
    fn test_runner_with_config() {
        let renderer = create_test_renderer();
        let config = HarnessConfig::default().with_tolerance(5).with_max_diff(1.0);
