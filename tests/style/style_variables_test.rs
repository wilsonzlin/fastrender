...
fn test_resolution_with_fallback() {
  ...
  assert_eq!(
    vars.resolve("var(--undefined, fallback)"),
    Some("fallback".to_string())
  );

  // Defined ignores fallback
  assert_eq!(
    vars.resolve("var(--defined, fallback)"),
...
fn test_design_token_hierarchy() {
  ...
  assert_eq!(
    component.resolve("var(--spacing-sm)"),
    Some("4px".to_string())
  );
  assert_eq!(
    component.resolve("var(--color-action)"),
...
fn test_fallback_with_whitespace() {
  ...
  assert_eq!(
    vars.resolve("var(--undefined,   blue   )"),
    Some("blue".to_string())
  );
}
