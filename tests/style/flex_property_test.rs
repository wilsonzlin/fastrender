...
fn styled_children(html: &str) -> Vec<fastrender::style::cascade::StyledNode> {
    ...
    let stylesheet = parse_stylesheet("flex-test", r#""#, None).unwrap();
    let styled = apply_styles_with_media(&dom, &stylesheet, &MediaContext::screen(800.0, 600.0));
    styled.children[0].children.clone()
}

#[test]
fn flex_grow_ignores_negative_numbers() {
    let children = styled_children(r#"<div><span style="flex-grow: -1"></span><span style="flex-grow: 2"></span></div>"#);
...
fn flex_shrink_ignores_negative_numbers() {
    let children = styled_children(r#"<div><span style="flex-shrink: -3"></span><span style="flex-shrink: 0.5"></span></div>"#);
