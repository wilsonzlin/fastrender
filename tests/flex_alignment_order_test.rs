...
fn align_self_respects_in_flow_child_order() {
    ...
    let mut start_child = BoxNode::new_block(Arc::new(start_style), FormattingContextType::Block, vec![]);
    start_child.id = 2;
    ...
    for child in &fragment.children {
        ...
        debug_children.push((id, child.bounds.x(), child.bounds.y(), child.bounds.width(), child.bounds.height()));
        match id {
    ...
    assert!((end_y - 30.0).abs() < 1e-3, "end child should align to flex-end");
    assert!((start_y - 0.0).abs() < 1e-3, "start child should remain at the cross-axis start");
}
