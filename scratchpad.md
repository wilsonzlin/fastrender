...
## Current issues / gaps
- Bidi: splits still reshaped without ligature preservation; nested inline wrappers/replaced items are modeled as U+FFFC during reordering (structural ordering rather than true inline content); `unicode-bidi` isolation is approximated with control characters, but we don't build explicit isolate/embedding stacks for box boundaries or honor `plaintext` base-detection beyond FSI.
