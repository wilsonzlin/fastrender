...
## Current issues / gaps
- Bidi: nested inline wrappers/replaced items are still modeled as U+FFFC during reordering (structural ordering rather than true inline content); `unicode-bidi` isolation is approximated with control characters, but we don't build explicit isolate/embedding stacks for box boundaries or honor `plaintext` base-detection beyond FSI.
