...
- Added newsweek.com to fetch_pages targets. Fetch succeeds (~1.09MB HTML, 21 CSS links). After skipping intrinsic image fetches, cascade ~5.8s, box_tree ~0.4s (3.2k boxes), but render still times out during layout/paint (likely carousel/flex heavy). Needs layout/perf follow-up.
Newsweek.com current status: fetch succeeds (~1.09MB HTML, 21 CSS). After skipping intrinsic image fetches, cascade ~5.9s, box_tree ~0.4–0.46s (3.2k boxes), but render still times out during layout/paint even with 120s timeout (many ContentTile carousels/flex). Needs layout/perf work (flex/carousel hot path).
- Added regressions:
