...
Absolute/fixed elements with both left/right insets and width:auto now use the constraint equation instead of shrink-to-fit, so inset overlays fill the containing block; regression `fixed_positioned_inset_auto_width_fills_viewport` covers a fixed bar spanning the viewport.
...
Added engadget.com to fetch_pages targets.
