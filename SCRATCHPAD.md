...
- newsweek.com fetch succeeds (~1.09MB HTML, 21 CSS links, no inline styles). Cascade ~6.5s (candidates ~715k, matches ~27k), but box_tree alone takes ~55s and render still times out before layout/paint complete. Likely heavy image/replaced processing; needs box_tree perf work.
