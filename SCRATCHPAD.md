...
Current hotspots to target: product-zone__inner and container_vertical-shelf-carousel wrappers/fields (multi-second flex layouts, repeated). Explore better layout cache reuse or skipping offscreen/hidden carousels to reduce the ~78s layout cost.
Flex histogram (FASTR_FLEX_PROFILE_HIST=1) shows heavy reuse of a few cache keys (e.g., bucket 4 key Some(3298164736) ~40k hits), indicating repeated similar probes/layouts for specific carousels/zones. Need to map keys back to nodes to reduce redundant layouts.
