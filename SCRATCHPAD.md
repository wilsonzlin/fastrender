...
Current task (Agent 6): investigating wikipedia.org blank/white render. Fetched `wikipedia.org` (cache under fetches/html/). Rendered via `cargo run --bin render_pages -- --pages wikipedia.org --timeout 120` producing fetches/renders/wikipedia.org.png. DOM parse now flips `no-js`→`js-enabled` and injects `jsl10n-visible` on both the HTML element and BODY (mirroring the portal script); regressions cover the root and body class injection (new `parse_html_marks_body_jsl10n_visible`). Latest release render after the body class addition still yields a mostly white image; next steps: inspect fragment positions/styles to ensure central-featured content becomes visible and consider handling overlay visibility defaults.
Caching metadata now preserves the source URL alongside content-type: fetch_pages writes `url:` and `content-type:` into `.html.meta`, and render_pages/fetch_and_render parse both (with a legacy fallback) to seed the base URL. This keeps relative assets (e.g., portal images/scripts) resolving to the original origin even when rendering cached `file://` HTML.
